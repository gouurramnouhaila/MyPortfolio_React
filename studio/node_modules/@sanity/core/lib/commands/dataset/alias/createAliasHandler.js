...
var _default = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (args, context) {
    ...
    var _yield$Promise$all = yield Promise.all([client.datasets.list().then(sets => sets.map(ds => ds.name)), aliasClient.listAliases(client).then(sets => sets.map(ds => ds.name)), client.request({
      uri: '/features'
    })]),
        _yield$Promise$all2 = _slicedToArray(_yield$Promise$all, 3),
        datasets = _yield$Promise$all2[0],
        aliases = _yield$Promise$all2[1],
        projectFeatures = _yield$Promise$all2[2];
    ...
    if (targetDataset) {
      var datasetErr = (0, _validateDatasetName.default)(targetDataset);
    ...
    var canCreateAlias = projectFeatures.includes('advancedDatasetManagement');
    ...
    try {
      yield aliasClient.createAlias(client, aliasName, datasetName);
